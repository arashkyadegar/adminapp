...
import { brandFormFilled } from "../../redux/store/brand/brand-form";
...
import { getBrandAction, submitEditBrandAction } from "../../redux/store/brand/brand-action";
import { useEffect } from "react";

export default function BrandEditComponent() {
  const { id } = useParams();
  ...
  function submit(event: any) {
    ...
    if (brandFormState.data.formIsValid) {
      ...
      try {
        dispatch(submitEditBrandAction(entity));
      } catch (err) {
        console.log("rrrr");
      }
    } else {
  ...
  return (
    <div className="w-full sm:w-11/12 mr-0 sm:mr-16">
      ...
      <div className="flex flex-col justify-start items-end">
        <LgSubmitbtnComponent title="ویرایش  برند" onClickFunc={submit}>
