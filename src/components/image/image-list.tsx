...
import { getImagesAction, searchImagesAction, submitDeleteImageAction } from "../../redux/store/image/image-action";
import { produce } from "immer";
import { imagesToggled } from "../../redux/store/image/images";
...
export default function ImageListComponent() {
  const imagesState = useAppSelector((state) => state.entities.images);
  const searchInputRef = useRef<HTMLInputElement>(null);
  const dispatch = useAppDispatch();
  useEffect(() => {
    dispatch(getImagesAction(1));
  }, []);
  ...
  function nextpage(page: number) {
    if (searchInputRef.current !== null) {
      ...
      if (name != "")
        dispatch(searchImagesAction(name, page));
      else
  ...
  function submitDeleteCategory(id: any) {
    if (window.confirm("قصد حذف دسته را دارید ؟ ")) {
      dispatch(submitDeleteImageAction(id));
    }
  }
  ...
  function submitSearchCategory() {
    if (searchInputRef.current !== null) {
      let name = searchInputRef.current.value;
      if (name != "")
        dispatch(searchImagesAction(name, 1));
      else
        dispatch(getImagesAction(1));
    }
  }
  ...
  return (
    <div className="w-full sm:w-10/12 mr-0 sm:mr-16">
      ...
      <div className=" flex flex-col p-4 bg-[#f8f9fa] ">
        <BoxTitleLgComponent title="لیست تصاویر " />
          ...
          <div className="flex flex-col w-full bg-white  border border-gray-200">
            <div className="flex flex-col sm:flex-row gap-2 justify-between p-4">
              <div className="flex flex-row gap-2 ">
                ...
                {/* refresh button  */}
                <button id="countries" onClick={() => { dispatch(getImagesAction(1)) }} className="flex sm:hidden bg-gray-50 border border-gray-300 text-gray-300 items-center justify-center hover:text-gray-700 text-sm rounded-lg w-fit  p-2.5 ">
            ...
            <div className="relative overflow-x-auto shadow-md border border-gray-200 p-4" >
              <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr className="flex flex-row">
                    ...
                    <th scope="col" className="flex text-xs items-center justify-center w-60">
                      عنوان
                    </th>
                ...
                <tbody>
                  {imagesState.list.map((item: any, index: number) => (
                    <tr
                    ...
                    >
                      ...
                      <th scope="col" className="flex items-center justify-center w-16">
                        <img src={getDefaultImageAvator(item.name)}
                          width="40rem" height="40rem"
                          loading="lazy" />
                      </th>
                      <td scope="row" className="flex items-center justify-center w-60">
                        {item.name}
                      </td>
              ...
              {/* <div className="flex flex-row-reverse justify-center gap-2 p-2">
                <>
                  {(() => {
                    const arr = [];
                    for (let i = 1; i <= (Math.ceil(images.totalCount / 10)); i++) {
